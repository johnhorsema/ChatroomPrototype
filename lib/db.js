...
module.exports.findUserMessages = function (userId, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('messages').filter({'from': userId}).orderBy(r.desc('timestamp')).run(connection, function(err, cursor) {
      if(err) {
        logerror("[ERROR][%s][findUserMessages] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
        callback(null, []);
        connection.close();
      }
      else {
        cursor.toArray(function(err, results) {
          if(err) {
            logerror("[ERROR][%s][findUserMessages][toArray] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
            callback(null, []);
          }
          else {
            callback(null, results);
